...
locals {
  ...
  # Same URL shape as the live via-supervisor-dso service header.
  cloud_run_urls = {
...
module "cloud_run" {
  ...
  # Match live via-supervisor-dso: only these env vars. Everything else is in GCS JSON.
  env_vars = merge(
    {
      VIA_CONFIG_BACKEND    = "gcp"
      GOOGLE_CLOUD_PROJECT  = var.project_id
      GOOGLE_CLOUD_LOCATION = var.region
      AGENT_URL             = local.cloud_run_urls[each.key]
      CONFIG_RELOAD_ID      = var.image_tag
    },
    each.value.env_vars,
  )
