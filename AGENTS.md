...
- via-supervisor deploys the DSO stack in `freyr-ai` / `us-east4`: Cloud Run (supervisor + superagent), two config buckets, six secrets, three Cloud SQL instances, on `freya-ai-dev-vpc`.
...
- Resource names follow `<product_name>-<environment>-<resource>` unless the env tfvars set an explicit DSO name (example: `via-supervisor-dso`).
